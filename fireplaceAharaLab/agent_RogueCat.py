...
def RogueCat_Condition(game, card, target):
	...
	if '自分の手札にドラゴンがいる場合' in dscrpt:
		...
		if target!=None and target.type == CardType.HERO:
			cond1 += 2
	elif '味方に海賊がいる' in dscrpt and '味方の海賊' in dscrpt:
		for card in player.characters:
			if card.race == Race.PIRATE:
				cond1 += 2
	elif '激励' in dscrpt:#ヒーローパワーを使うと発動する
		pass
...
#<b>沈黙</b>させる->「攻撃できない」が自分の場に出ていたら
	...
	if '自分の手札に追加する' in dscrpt:
		cond2 += 2
	elif '相手のデッキに' in dscrpt and  '混ぜる' in dscrpt:
		cond2 += 2
